...
static AsyncCallbackJsonWebHandler *handler =
    new AsyncCallbackJsonWebHandler("/json2");
...
void drawDeviceMode3() {
    ...
    sprite.unloadFont();
    StickCP2.Display.pushImage(0, 0, MY_WIDTH, MY_HEIGHT,
...
void drawDeviceModeConfig(uint8_t _desired_devicemode) {
    ...
    if (StickCP2.BtnB.wasPressed()) {
        ...
        switch (desired_devicemode) {
            case 1:
                desired_devicemode = 2;
                break;
            case 2:
                desired_devicemode = 3;
                break;
            case 3:
                desired_devicemode = 4;
                break;
            case 4:
                desired_devicemode = 1;
                break;
        }
    }
    if (StickCP2.BtnA.wasPressed()) {
...
void drawBatt() {
    ...
    int batteryPercent =
        map(vol, 3000, 4200, 0, 100);  // Convert voltage to percentage
...
void drawTime() {
    // time
    struct tm timeinfo;
    static constexpr const char *const wd_en[7] = {"Sun", "Mon", "Tue", "Wed",
                                                   "Thu", "Fri", "Sat"};
    static constexpr const char *const wd_nl[7] = {"Zon", "Maa", "Din", "Woe",
                                                   "Don", "Vri", "Zat"};
    auto dt                                     = StickCP2.Rtc.getDateTime();
    ...
    snprintf(timebuffer, sizeof(timebuffer), "%02d:%02d", dt.time.hours,
             dt.time.minutes);
...
void drawMain() {
    ...
    sprite.fillSmoothRoundRect(10, 10, picto_box_width, picto_box_height, 5,
                               FG_COLOR, BG_COLOR);
    ...
    // by default, if there is no current activity, the first one will be
    // current
    ...
    if (config_activities_size < config_activities_size_max) {
        for (int i = 0; i < config_activities_size; i++) {
            ...
            if (i == ps_current_activity_index) {
                sprite.fillRect(_circle_x - 8, 129, 16, 4,
                                DAYPERIOD_CIRCLE_BG_COLOR);
            }
            if (get_pspref_activity_done(i) == 1) {
                sprite.fillSmoothCircle(_circle_x, 122, _size_circle,
                                        COLOR_DONE, BG_COLOR);
            } else {
                sprite.fillSmoothCircle(_circle_x, 122, _size_circle,
                                        COLOR_TODO, BG_COLOR);
    ...
    if (devicemode = 4) {
        if (StickCP2.BtnPWR.wasPressed()) {
            ...
            if (ps_current_activity_index >= 1) {
                set_pspref_current_activity_index(ps_current_activity_index -
                                                  1);
            } else {
                if (get_pspref_buzzer()) {
                    StickCP2.Speaker.tone(6000, 100);
...
void setup() {
    ...
    bool dmInit =
        dmPrefs.isKey("devicemode");  // Test for the existence
